...
**Stack:** Eleventy · LiquidJS · LightningCSS · markuplint · Husky + lint-staged · pnpm

## Dev commands

- `pnpm dev` — Eleventy dev server with live reload
- `pnpm build` — build site (Eleventy + LightningCSS minification)
- `pnpm lint` — markuplint on source templates in `src/`
- `pnpm test:links` — linkinator on built `_site/`

## Development rules

- Always use `pnpm add -D <package>` — never `npx` for installing.
- Validate source files in `src/`, not the generated output in `_site/`.
- Don't invoke devDependency binaries via `npx` in `package.json` scripts or hooks — reference the binary name directly (pnpm resolves `.bin` executables automatically).
